...
import { PARALLEL_SELF_PRESETS, ParallelSelfPreset } from '../src/presets/parallelSelf';

interface ParallelSelfModeProps {
  onGenerate: (presetId?: string, customPrompt?: string) => void;
}

function ParallelSelfMode({ onGenerate }: ParallelSelfModeProps) {
  const [selectedPreset, setSelectedPreset] = useState<string | null>(null);
  const [presetAnims] = useState<{ [key: string]: Animated.Value }>({});
  const [isProcessing, setIsProcessing] = useState(false);

  // Use centralized presets from website
  const parallelSelfPresets = PARALLEL_SELF_PRESETS.map((preset: ParallelSelfPreset) => ({
    id: preset.id,
    label: preset.label,
    description: preset.features?.join(', ') || 'Parallel transformation',
  ...
  const handlePresetClick = (preset: { id: string; label: string; description: string; prompt: string }) => {
    ...
    if (isProcessing) {
      console.log('Parallel Self preset click ignored - already processing');
      return;
    }
    
    console.log('Parallel Self preset clicked:', preset.id);
      ...
      <View style={styles.presetContainer}>
        <View style={styles.presetGrid}>
          ...
          <View style={styles.presetRow}>
            {parallelSelfPresets.slice(0, 3).map((preset) => (
              <Animated.View 
                key={preset.id}
                style={[
          ...
          <View style={styles.presetRow}>
            {parallelSelfPresets.slice(3).map((preset) => (
              <Animated.View 
                key={preset.id}
                style={[
                  styles.presetButtonWrapper,
...
export default function GenerateParallelSelfScreen() {
  const { mode } = useLocalSearchParams();
  return (
    <BaseGenerationScreen mode="parallel-self">
      {({ onGenerate }) => (
        <ParallelSelfMode onGenerate={onGenerate} />
      )}
